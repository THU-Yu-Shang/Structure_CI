...
class Model(nn.Module):
    def __init__(self, node_num, p, k, m, in_channels, out_channels, graph_mode, model_mode, dataset_mode, is_train):
        ...
        if self.model_mode is "CIFAR10":
            ...
            self.SMALL_conv3 = nn.Sequential(
                RandWire(self.node_num, self.p, self.k, self.m, self.in_channels, self.out_channels, self.graph_mode, self.is_train, name="SMALL_conv3")
            )
            self.SMALL_conv4 = nn.Sequential(
                RandWire(self.node_num, self.p, self.k, self.m, self.in_channels, self.out_channels * 2, self.graph_mode, self.is_train, name="SMALL_conv4")
            )
            self.SMALL_conv5 = nn.Sequential(
                RandWire(self.node_num, self.p, self.k, self.m, self.in_channels * 2, self.out_channels * 4, self.graph_mode, self.is_train, name="SMALL_conv5")
            )
